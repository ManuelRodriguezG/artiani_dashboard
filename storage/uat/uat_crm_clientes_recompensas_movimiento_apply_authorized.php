 ...
 * IA: Codex GPT-5
 * Fecha: 2026-06-30
 * Proposito: aplicar movimiento CRM Recompensas solo con token y respaldo externo.
 * Impacto: inserta movimiento manual/controlado y actualiza saldo.
 * Contrato: no conecta POS/Ventas; bloqueado por defecto.
 */
...
$opciones = getopt("", array("autorizar::", "respaldo::", "cliente::", "programa::", "tipo::", "puntos::", "descripcion::", "origen_id::"));
...
$validacion = validar_respaldo_crm_recompensas_movimiento($respaldo);

if ($autorizar !== "CRM_CLIENTES_RECOMPENSAS_MOVIMIENTO" || !$validacion["ok"]) {
  echo json_encode(array(
    "error" => true,
    "tipo" => "warning",
    "mensaje" => "No se creo movimiento CRM Recompensas. Falta token o respaldo valido.",
    "depurar" => array(
      "requerido" => array(
        "autorizar" => "CRM_CLIENTES_RECOMPENSAS_MOVIMIENTO",
        "respaldo" => "RUTA_O_REFERENCIA"
      ),
      "validacion_respaldo" => $validacion,
      "alcance" => array(
        "crea_movimiento_manual" => true,
        "actualiza_saldo_cuenta" => true,
        "conecta_pos_ventas" => false,
        "modifica_cliente" => false,
        "toca_ecommerce_garantias_apartados_devoluciones_legacy" => false
      )
    )
...
$datos = array(
  ...
  "puntos" => isset($opciones["puntos"]) ? floatval($opciones["puntos"]) : 0,
  "origen_modulo" => "crm",
...
echo json_encode((new ClientesCrm())->recompensaMovimientoCrearAutorizado($datos), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) . PHP_EOL;

function validar_respaldo_crm_recompensas_movimiento($respaldo) {
