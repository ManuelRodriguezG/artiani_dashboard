 ...
 * IA: Codex GPT-5
 * Fecha: 2026-06-30
 * Proposito: aplicar DDL CRM Recompensas solo con token y respaldo externo.
 * Impacto: crea tablas de programas, cuentas y movimientos de recompensas CRM.
 * Contrato: no crea programas reales, cuentas, movimientos ni puntos; requiere respaldo externo valido.
 */
...
class UatCrmClientesRecompensasSchemaApplyAuthorized extends CRUD {
  const TOKEN = "CRM_CLIENTES_RECOMPENSAS_DDL";
  ...
  public function ejecutar($token, $respaldo) {
    ...
    if (!$validacion["ok"]) {
      return array(
        "error" => true,
        "tipo" => "warning",
        "mensaje" => "Autorizacion incompleta; no se ejecuto DDL CRM Recompensas.",
        "depurar" => array(
    ...
    $respuesta = $modelo->planActualizarRecompensasClientesCrm(true);
    $postflight = $this->estadoTablas();

    return array(
      "error" => !empty($respuesta["error"]),
      "tipo" => empty($respuesta["error"]) ? "success" : "danger",
      "mensaje" => empty($respuesta["error"]) ? "DDL CRM Recompensas aplicado." : "No se pudo aplicar DDL CRM Recompensas.",
      "depurar" => array(
  ...
  private function alcanceSeguro() {
    return array(
      "crea_tablas_recompensas" => $this->tablas,
      "crea_programas_reales" => false,
      "crea_cuentas" => false,
      "otorga_puntos" => false,
      "redime_puntos" => false,
...
echo json_encode((new UatCrmClientesRecompensasSchemaApplyAuthorized())->ejecutar($token, $respaldo), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) . PHP_EOL;
