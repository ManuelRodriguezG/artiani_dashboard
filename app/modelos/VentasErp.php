...
class VentasErp extends CRUD {
    ...
    public function ventaInventarioPendienteDryRun($datos = array()) {
        try {
            ...
            } else if ($cantidadCubierta > 0) {
                $estado = "pendiente_autorizable";
                $advertencias[] = "Flujo mixto: la parte disponible debe descontarse con kardex y solo el faltante queda como pendiente de Inventario/Existencias.";
            ...
            return $this->respuesta(false, $estado === "bloqueado" ? "warning" : "success", "Dry-run de inventario pendiente POS generado", array(
                ...
                "cantidad_cubierta" => $cantidadCubierta,
                "cantidad_pendiente" => $cantidadPendiente,
                "flujo_mixto" => $cantidadCubierta > 0 && $cantidadPendiente > 0,
                "contrato_flujo_mixto" => array(
                    "descontar_cantidad_cubierta_con_kardex" => $cantidadCubierta > 0 && $cantidadPendiente > 0,
                    "crear_pendiente_solo_por_faltante" => $cantidadPendiente > 0,
                    "validar_transaccionalmente_existencia_cubierta" => $cantidadCubierta > 0 && $cantidadPendiente > 0
                ),
                "politica" => array(
     ...
     * Impacto: crea venta, detalle, pago, movimiento de caja, expediente pendiente, evento y notificacion; en flujo mixto descuenta la parte cubierta con kardex.
     * Contrato: transaccional; requiere turno abierto, pago completo y politica POS para el faltante. Si falta caja/turno/pago no escribe datos.
     */
    public function ventaInventarioPendienteReal($datos = array()) {
        ...
        try {
            ...
            $db->beginTransaction();
            $this->asegurarTransaccionPosReal($db, "inicio_flujo_pendiente");
            $turnoBloqueado = $this->bloquearTurnoPosReal($db, $idTurno, $idCaja, $idAlmacen);
            $this->asegurarTransaccionPosReal($db, "turno_bloqueado");
            if (!$turnoBloqueado) {
                throw new Exception("El turno ya no esta abierto para la caja asignada");
            }
            ...
            $this->asegurarTransaccionPosReal($db, "venta_insertada");

            $modoSalidaPendiente = $cantidadCubierta > 0 ? "mixto_kardex_pendiente_pos" : "inventario_pendiente_pos";
            $estadoInventarioDetalle = $cantidadCubierta > 0 ? "pendiente_inventario_parcial" : "pendiente_inventario";
            $stmt = $db->prepare("INSERT INTO erp_ventas_detalle
                ...
                VALUES (:venta, 1, :producto, :sku_id, :sku, :descripcion,
                 'producto', 1, :modo_salida, :inventario_estado, 1,
            ...
            $stmt->execute(array(
                ...
                ":sku" => $this->valor($sku, "sku", ""),
                ":descripcion" => $this->valor($sku, "nombre_sku", $this->valor($sku, "producto", "")),
                ":modo_salida" => $modoSalidaPendiente,
                ":inventario_estado" => $estadoInventarioDetalle,
                ":cantidad" => $cantidad,
            ...
            if ($cantidadCubierta > 0) {
                ...
                if (round(floatval($this->valor($planCubierto, "faltante", 0)), 6) > 0) {
                    throw new Exception("Existencia cubierta insuficiente al confirmar venta POS con inventario pendiente mixto");
                }
                foreach ($this->valor($planCubierto, "asignaciones", array()) as $asignacionInv) {
                    $evidenciaInventarioCubierta[] = $this->aplicarSalidaInventarioPosReal($db, $idVenta, $idDetalle, $folio, $sku, $asignacionInv, $idAlmacen, $idUsuario);
                    $this->asegurarTransaccionPosReal($db, "kardex_cubierto_insertado");
                }
            }
            ...
            $snapshot = array(
                ...
                "kardex_cubierto" => $evidenciaInventarioCubierta
            );
            ...
            $this->asegurarTransaccionPosReal($db, "pendiente_insertado");
            ...
            $this->asegurarTransaccionPosReal($db, "detalle_ligado_pendiente");
            ...
            $this->asegurarTransaccionPosReal($db, "detalle_inventario_pendiente_insertado");
            ...
            $this->asegurarTransaccionPosReal($db, "evento_pendiente_insertado");
            $idNotificacionPendiente = $this->registrarNotificacionInventarioPendientePos(
                $db,
                $idPendiente,
                $folioPendiente,
                $folio,
            ...
            if (intval($idNotificacionPendiente) > 0) {
                $db->prepare("UPDATE erp_pos_inventario_pendientes
                    SET id_notificacion=:notificacion
                    WHERE id_inventario_pendiente=:pendiente")
                    ->execute(array(":notificacion" => intval($idNotificacionPendiente), ":pendiente" => $idPendiente));
            }
            $this->asegurarTransaccionPosReal($db, "notificacion_pendiente_registrada");
            ...
            $this->asegurarTransaccionPosReal($db, "pagos_registrados");
            ...
            $this->asegurarTransaccionPosReal($db, "turno_recalculado");
     ...
     * Proposito: crear expediente de inventario pendiente por partida dentro del cobro POS multipartida.
     * Impacto: liga venta/detalle con alerta a Inventario sin separar la cuenta ni mezclar faltantes sin trazabilidad.
     * Contrato: se invoca dentro de una transaccion de venta POS ya bloqueada; no confirma pagos por separado.
     */
    private function registrarPendienteInventarioDesdeVentaPosReal($db, $idVenta, $idDetalle, $folioVenta, $idAlmacen, $idSku, $sku, $cantidadVendida, $cantidadCubierta, $cantidadPendiente, $precioUnitario, $politica, $motivo, $idUsuario) {
        $cantidadPendiente = round(floatval($cantidadPendiente), 6);
        if ($cantidadPendiente <= 0) {
            return null;
        }
        if (!$this->tablaExiste($db, "erp_pos_inventario_pendientes")
            || !$this->tablaExiste($db, "erp_pos_inventario_pendientes_eventos")) {
            throw new Exception("Falta esquema POS de inventario pendiente para cobrar con faltantes");
        }
        if (!is_array($politica) || empty($politica)) {
            $politicaConsulta = $this->consultarPoliticaInventarioPendientePos(
                $db,
                $idSku,
                $idAlmacen,
                "pos",
                $cantidadPendiente,
                round($cantidadPendiente * floatval($precioUnitario), 6)
            );
            if (!empty($politicaConsulta["bloqueos"])) {
                throw new Exception("Politica POS de inventario pendiente ya no autoriza el faltante del SKU " . $this->valor($sku, "sku", ""));
            }
            $politica = $this->valor($politicaConsulta, "politica", array());
        }

        $folioPendiente = $this->generarFolioInventarioPendientePosReal($db);
        $snapshot = array(
            "venta" => array("id_venta" => intval($idVenta), "folio" => $folioVenta),
            "detalle" => array("id_venta_detalle" => intval($idDetalle)),
            "politica" => $politica,
            "origen" => "pos_ui_multipartida",
            "cantidades" => array(
                "cantidad_vendida" => round(floatval($cantidadVendida), 6),
                "cantidad_cubierta" => round(floatval($cantidadCubierta), 6),
                "cantidad_pendiente" => $cantidadPendiente
            )
        );
        ...
        $stmt->execute(array(
            ...
            ":cantidad_pendiente" => $cantidadPendiente,
            ":unidad" => $this->valor($sku, "unidad_venta_label", ""),
            ":precio" => round(floatval($precioUnitario), 6),
            ":politica" => json_encode($politica, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
            ":snapshot" => json_encode($snapshot, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
        ...
        $setsDetalle = array();
        $paramsDetalle = array(":detalle" => intval($idDetalle));
        if ($this->columnaExiste($db, "erp_ventas_detalle", "id_inventario_pendiente")) {
            $setsDetalle[] = "id_inventario_pendiente=:pendiente";
            $paramsDetalle[":pendiente"] = $idPendiente;
        }
        if ($this->columnaExiste($db, "erp_ventas_detalle", "inventario_estado")) {
            $setsDetalle[] = "inventario_estado=:estado";
            $paramsDetalle[":estado"] = round(floatval($cantidadCubierta), 6) > 0 ? "pendiente_inventario_parcial" : "pendiente_inventario";
        }
        if ($this->columnaExiste($db, "erp_ventas_detalle", "permite_inventario_pendiente")) {
            $setsDetalle[] = "permite_inventario_pendiente=1";
        }
        if ($this->columnaExiste($db, "erp_ventas_detalle", "cantidad_inventario_pendiente")) {
            $setsDetalle[] = "cantidad_inventario_pendiente=:cantidad_pendiente";
            $paramsDetalle[":cantidad_pendiente"] = $cantidadPendiente;
        }
        if (!empty($setsDetalle)) {
            $db->prepare("UPDATE erp_ventas_detalle SET " . implode(", ", $setsDetalle) . " WHERE id_venta_detalle=:detalle")
                ->execute($paramsDetalle);
        }
        ...
        $idNotificacion = $this->registrarNotificacionInventarioPendientePos(
            $db,
            $idPendiente,
            $folioPendiente,
            $folioVenta,
        ...
        if (intval($idNotificacion) > 0) {
            $db->prepare("UPDATE erp_pos_inventario_pendientes
                SET id_notificacion=:notificacion
                WHERE id_inventario_pendiente=:pendiente")
                ->execute(array(":notificacion" => intval($idNotificacion), ":pendiente" => $idPendiente));
        }
    ...
    /**
     * Documentacion IA: Codex GPT-5, 2026-07-12.
     * Proposito: indexar una venta POS con inventario pendiente en la bandeja global de notificaciones.
     * Impacto: crea/actualiza alerta operativa para Inventario; no mueve inventario ni altera la venta.
     * Contrato: idempotente por huella `pos_inventario_pendiente|id`.
     */
    private function registrarNotificacionInventarioPendientePos($db, $idPendiente, $folioPendiente, $folioVenta, $idAlmacen, $idSku, $sku, $cantidadPendiente, $idUsuario) {
        try {
            if (!$this->tablaExiste($db, "erp_notificaciones") || intval($idPendiente) <= 0) {
                return null;
            }
            $skuCodigo = $this->valor($sku, "sku", "");
            $descripcion = $this->valor($sku, "nombre_sku", $this->valor($sku, "producto", ""));
            ...
            $payload = array(
                ...
                "folio_pendiente" => $folioPendiente,
                "folio_venta" => $folioVenta,
                "id_inventario_pendiente" => intval($idPendiente),
                "id_almacen" => intval($idAlmacen),
                "id_sku" => intval($idSku),
                "sku" => $skuCodigo,
                "descripcion" => $descripcion,
                "cantidad_pendiente" => floatval($cantidadPendiente),
                "accion_sugerida" => "validar_conteo_fisico_en_inventario_existencias"
            );
            $payloadJson = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            $stmt = $db->prepare("SELECT id_notificacion
                FROM erp_notificaciones
                WHERE tipo='pos_venta_inventario_pendiente'
                  AND modulo_origen='ventas_pos'
                  AND entidad_origen='erp_pos_inventario_pendientes'
            ...
            if ($idNotificacion > 0) {
                $stmt = $db->prepare("UPDATE erp_notificaciones
                    SET area_responsable='inventario',
                        permiso_requerido='inventario.ver',
                        titulo='Venta POS con inventario pendiente',
                        descripcion=:descripcion,
                        prioridad='alta',
                        url_accion='/inventario/productos_existencias#pendientes-pos',
                        payload_json=:payload,
                        fecha_actualizacion=NOW()
                    WHERE id_notificacion=:notificacion");
                $stmt->execute(array(
                    ":descripcion" => "Validar existencia fisica del SKU " . $skuCodigo . " por venta " . $folioVenta . ".",
                    ":payload" => $payloadJson,
            ...
            $stmt = $db->prepare("INSERT INTO erp_notificaciones
                ...
                VALUES
                ('pos_venta_inventario_pendiente', 'ventas_pos', 'erp_pos_inventario_pendientes', :pendiente,
                 'inventario', 'inventario.ver', 'Venta POS con inventario pendiente', :descripcion,
                 'alta', 'pendiente', '/inventario/productos_existencias#pendientes-pos', :payload, :usuario, NULL)");
            $stmt->execute(array(
                ":pendiente" => intval($idPendiente),
                ":descripcion" => "Validar existencia fisica del SKU " . $skuCodigo . " por venta " . $folioVenta . ".",
                ":payload" => $payloadJson,
    ...
    public function confirmarVentaPosReal($datos = array()) {
        ...
        $autorizarInventarioPendiente = strtoupper(trim((string) $this->valor($datos, "autorizar_inventario_pendiente_pos", ""))) === "AUTORIZAR INVENTARIO PENDIENTE";
        $motivoInventarioPendiente = trim((string) $this->valor($datos, "motivo_inventario_pendiente", $this->valor($datos, "motivo", "")));

        if ($idUsuario <= 0) {
            return $this->respuesta(true, "warning", "Usuario operador obligatorio", array("bloqueos" => array("usuario_obligatorio")));
        }
        if ($autorizarInventarioPendiente && $motivoInventarioPendiente === "") {
            return $this->respuesta(true, "warning", "Motivo obligatorio para cobrar con faltantes", array("bloqueos" => array("motivo_inventario_pendiente_obligatorio")));
        }
        if (!$this->schemaVentaPosCompleto($db)) {
        ...
        try {
            ...
            foreach ($partidas as $partida) {
                ...
                if ($afectarInventarioVenta && $cantidadPendientePartida > 0) {
                    ...
                    $cantidadCubiertaPartida = round(max(0, floatval($this->valor($partida, "cantidad", 0)) - $cantidadPendientePartida), 6);
                    $pendienteInventario = $this->registrarPendienteInventarioDesdeVentaPosReal(
                        $db,
                        $idVenta,
                        $idDetalle,
                        $folio,
                        ...
                        $cantidadCubiertaPartida,
                        $cantidadPendientePartida,
                        $precioFinal,
                        $this->valor($planSalidaPartida, "politica_pos", null),
                        $motivoInventarioPendiente,
                        $idUsuario
                    );
                    $pendientesInventario[] = $pendienteInventario;
                    $cantidadInventarioPendienteTotal += $cantidadPendientePartida;
                }
            ...
            if ($cantidadInventarioPendienteTotal > 0) {
                $setsInventarioPendiente = array();
                $paramsInventarioPendiente = array(":venta" => $idVenta);
                if ($this->columnaExiste($db, "erp_ventas", "inventario_validacion_estado")) {
                    $setsInventarioPendiente[] = "inventario_validacion_estado='pendiente_inventario'";
                }
                if ($this->columnaExiste($db, "erp_ventas", "inventario_pendiente_total")) {
                    $setsInventarioPendiente[] = "inventario_pendiente_total=:pendiente_total";
                    $paramsInventarioPendiente[":pendiente_total"] = round($cantidadInventarioPendienteTotal, 6);
                }
                if (!empty($setsInventarioPendiente)) {
                    $db->prepare("UPDATE erp_ventas SET " . implode(", ", $setsInventarioPendiente) . " WHERE id_venta=:venta")
                        ->execute($paramsInventarioPendiente);
                }
            }
    ...
    private function resolverPrecioSkuDryRun($db, $sku, $cliente, $canal, $idAlmacen, $schemaListasPendiente) {
        ...
        $whereCliente = "";
        $selectCliente = "0 es_cliente, 0 prioridad_cliente, 9999 prioridad_cliente_tipo";
        if (($idClienteCrm > 0 || $idCliente > 0) && $this->tablaExiste($db, "erp_clientes_listas_precios")) {
            ...
            if (!empty($condicionesCliente)) {
                ...
                $prioridadClienteTipo = "2";
                if ($idClienteCrm > 0 && $tieneClienteCrmEnLista) {
                    $prioridadClienteTipo = "CASE
                        WHEN cl.id_cliente_crm=:cliente_crm THEN 0
                        WHEN cl.id_cliente=:cliente_crm_compat THEN 1
                        ELSE 2
                    END";
                } elseif ($idCliente > 0) {
                    $prioridadClienteTipo = "CASE WHEN cl.id_cliente=:cliente_erp THEN 0 ELSE 1 END";
                }
                $selectCliente = "CASE WHEN cl.id_cliente_lista_precio IS NOT NULL THEN 1 ELSE 0 END es_cliente,
                    ...
                    {$prioridadClienteTipo} prioridad_cliente_tipo";
            }
    ...
    private function prevalidarPartida($db, $item, $idAlmacen, $renglon, $cliente = array(), $canal = "pos", $schemaListasPendiente = true, $autorizarInventarioPendiente = false, $omitirAfectacionInventario = false) {
        ...
        if (!$omitirAfectacionInventario && intval($sku["controla_inventario"]) === 1 && $cantidad > floatval($disponibilidad["disponible"]) + 0.0001) {
            ...
            } elseif ($autorizarInventarioPendiente) {
                $planSalida["inventario_pendiente_autorizado"] = true;
                $planSalida["cantidad_pendiente"] = $cantidadPendiente;
                $planSalida["cantidad_cubierta"] = round(max(0, $cantidad - $cantidadPendiente), 6);
                $planSalida["politica_pos"] = $this->valor($politicaPendiente, "politica", null);
            } else {
    ...
    /**
     * Documentacion IA: Codex GPT-5, 2026-07-12.
     * Proposito: detectar perdida de transaccion durante flujos POS con inventario/caja antes de confirmar.
     * Impacto: no escribe datos; convierte fallos silenciosos en mensajes con paso diagnostico para evitar duplicidades.
     * Contrato: debe llamarse solo dentro de flujos que ya ejecutaron beginTransaction().
     */
    private function asegurarTransaccionPosReal($db, $paso) {
        if (!$db instanceof PDO || !$db->inTransaction()) {
            throw new Exception("Transaccion POS inactiva en paso: " . $paso);
        }
    }
