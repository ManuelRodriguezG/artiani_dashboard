...
class Costos extends CRUD {
    ...
    private $diferencia;
    private $porcentaje_diferencia;
    private $requiere_revision;
    private $fch_m = DATE_NOW;
...
//LEFT JOIN crm_clientes crmc ON crmc.id_cliente = ecomp.id_cliente
        $this->setColumnas(array(
            "erpplphc.id_historial",
            "erpplphc.id_proveedor",
            'erpplphc.sku',
            "erpplphc.costo_anterior",
            "erpplphc.costo_nuevo",
            "erpplphc.precio_actual",
            "erpplphc.precio_sugerido",
            "erpplphc.diferencia",
            "erpplphc.porcentaje_cambio",
            "erpplphc.fch_m",
            "erpp.proveedor",
            "ecomp.nombre",
            "ecompi.url_imagen"
        ));
        $this->setTabla($this->tabla_erp_proveedores_listas_productos_historial_costos." erpplphc");
        $this->setLeftJoin($this->tabla_proveedores . " erpp ON erpp.id_proveedor = erpplphc.id_proveedor");
        $this->setLeftJoin($this->tabla_ecom_productos . " ecomp ON ecomp.sku = erpplphc.sku");
        $this->setLeftJoin($this->tabla_ecom_productos_imagenes . " ecompi ON ecompi.id_producto = ecomp.id_producto");
        $this->setWhere("requiere_revision = 1");
        $this->setAnd("ecompi.tipo_imagen = 'portada'");
        $this->setOrderBy("fch_m");
...
//    var_dump($valores_registrar);
    ...
    public function quitar_revision_historial_costo() {
        $campos = array(
            "requiere_revision"
        );
        $valores = array(
            $this->getRequiere_revision()
        );
        $this->setColumnas($campos);
        $this->setColumnasValores($valores);
        $this->setWhere('id_historial = ' . $this->getId_historial());
...
//    var_dump($valores_registrar);
    ...
    public function getRequiere_revision() {
        return $this->requiere_revision;
    }

    public function setRequiere_revision($requiere_revision): void {
        $this->requiere_revision = $requiere_revision;
    }
    public function getId_historial() {
        return $this->id_historial;
    }

    public function setId_historial($id_historial): void {
