...
class VentasErpEsquema extends DBSchema {
     ...
     * Proposito: preparar estructura formal de configuracion de ticket POS.
     * Impacto: define datos de empresa y preferencias de impresion por almacen/caja/terminal; no modifica ventas ni tickets historicos.
     * Contrato: con $ejecutar=false solo genera SQL; con true requiere autorizacion externa, respaldo vigente y confirmacion exacta.
     */
    public function planActualizarTicketConfigPos($ejecutar = false) {
        $opciones = "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
        $plan = array();

        $plan[] = $this->crearTablaSiNoExiste("erp_empresa_configuracion", array(
        ...
        $plan[] = $this->crearTablaSiNoExiste("erp_pos_ticket_configuracion", array(
        ...
        $plan[] = $this->agregarColumnaSiNoExiste("erp_ventas", "ticket_config_snapshot", "TEXT NULL AFTER `cliente_snapshot`", $ejecutar);
        $plan[] = $this->agregarIndiceSiNoExiste("erp_ventas", "idx_ventas_ticket_fecha", "KEY `idx_ventas_ticket_fecha` (`id_almacen`, `id_caja`, `fecha_venta`)", $ejecutar);
    ...
    /**
     * Documentacion IA: Codex GPT-5, 2026-07-24.
     * Proposito: auditar estructura requerida para configuracion formal de ticket POS.
     * Impacto: solo lectura sobre INFORMATION_SCHEMA; no crea empresa, no cambia ventas y no configura hardware.
     * Contrato: devuelve faltantes para autorizar DDL y mantiene separado contenido, formato e impresora.
     */
    public function auditarTicketConfigPos() {
        $tablas = array("erp_empresa_configuracion", "erp_pos_ticket_configuracion", "erp_ventas");
        $columnas = array(
            "erp_empresa_configuracion" => array("id_empresa_configuracion", "clave_empresa", "nombre_comercial", "razon_social", "rfc", "regimen_fiscal", "direccion_fiscal", "telefono", "whatsapp", "email", "sitio_web", "logo_url", "leyenda_ticket_general", "leyenda_no_fiscal", "leyenda_devoluciones", "leyenda_garantias", "estatus", "creado_por", "actualizado_por", "fecha_registro", "fecha_actualizacion"),
            "erp_pos_ticket_configuracion" => array("id_ticket_configuracion", "id_empresa_configuracion", "id_almacen", "id_caja", "id_terminal_pos", "nombre_configuracion", "prioridad", "ticket_ancho_mm", "ticket_columnas", "fuente", "mostrar_logo", "logo_modo", "impresion_modo", "impresora_nombre_windows", "corte_automatico", "abrir_cajon", "copias_venta", "copias_devolucion", "margen_mm", "qr_ticket", "mensaje_sucursal", "estatus", "creado_por", "actualizado_por", "fecha_registro", "fecha_actualizacion"),
            "erp_ventas" => array("id_venta", "folio", "id_almacen", "id_caja", "fecha_venta", "ticket_config_snapshot")
        );
        $indices = array(
            "erp_empresa_configuracion" => array("idx_empresa_config_clave", "idx_empresa_config_estatus"),
            "erp_pos_ticket_configuracion" => array("idx_pos_ticket_empresa", "idx_pos_ticket_scope", "idx_pos_ticket_modo"),
            "erp_ventas" => array("idx_ventas_ticket_fecha")
        );
        $resultado = array("tablas" => array(), "columnas" => array(), "indices" => array());
        foreach ($tablas as $tabla) {
            $resultado["tablas"][] = array("tabla" => $tabla, "existe" => $this->tablaExiste($tabla));
        ...
        return array(
            "error" => false,
            "tipo" => "success",
            "mensaje" => "Auditoria configuracion ticket POS generada",
            "depurar" => $resultado,
