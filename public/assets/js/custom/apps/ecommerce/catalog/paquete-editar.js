                ...
                validator.validate().then(async function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        if (informacion.productos.length > 0) {
                            let response = await actualizar_paquete(informacion);
                            if (response.error == false) {
                                if ($("#avatar")[0].files.length > 0) {
                                    ...
                                    formSubirPortada.append('id_paquete', informacion.id_paquete);
...
KTUtil.onDOMContentLoaded(function () {
    ...
    let response = paquete_a_editar();
    if (response['error'] == false) {
        ui_datos_formulario(response.depurar.info.depurar);
        ui_pedido_productos(response.depurar.productos);
...
function paquete_a_editar() {
    let id_paquete = obtener_id_paquete();
    let data = {
        id_paquete: id_paquete
    }
    let consulta = JSON.parse(consultar_paquete_editar(data));
