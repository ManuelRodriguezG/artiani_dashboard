...
// Init DropzoneJS --- more info:
    const initDropzone = () => {
        ...
        var myDropzone = new Dropzone("#kt_ecommerce_add_product_media", {
            url: "/producto/imagenes_complementarias", // Set the url for your upload script location
            paramName: "file", // The name that will be used to transfer the file
            params: {
                'id_producto': id_producto,
                'tipo_imagen': 'complementaria'
            },
            maxFiles: 10,
            maxFilesize: 10, // MB
            addRemoveLinks: true,
            ...
            success: function (file, response) {
                ...
                if (respuesta.error == false) {
                    ...
                    let code = `<div class="col-2" id="contenedor-imagen-complementaria-${respuesta.depurar.id_registro}">
                                                                        <div>
                                                                            <img src="${respuesta.depurar.url_imagen}" width="100%">
                                                                        </div>
                                                                        <div>
                                                                            <a class="form-control primary" href="javascript:void(0);" onclick="eliminar_imagen_complementaria(this)" ruta_imagen="${respuesta.depurar.url_imagen}" id="imagen-complementaria-${respuesta.depurar.id_registro}">
                                                                                eliminar
                                                                            </a>
                                                                        </div>  
                                                                    </div>`;
                    $('#galeria-imagenes-producto').append(code);
                }
            }
        });
    }
...
//    ui_proveedores_producto(consulta.depurar.proveedores.depurar);
    ...
    if (response.depurar.sugeridos.error == false) {
        ui_sugeridos(response.depurar.sugeridos.depurar);
    }
    //complementarios
    if (response.depurar.complementos.error == false) {
        ui_complementos(response.depurar.complementos.depurar);
    }
...
async function eliminar_imagen_complementaria(elemento) {
    console.log($(elemento));
    let id = $(elemento)[0].id.substring(22);
    let ruta_imagen = $(elemento).attr('ruta_imagen');
    console.log(id);
    let data = {
        id_imagen: id,
        ruta_imagen
    }
    let respuesta = await eliminar_complementaria(data);
    if (respuesta.error == false) {
        $('#contenedor-imagen-complementaria-' + id).remove();
    }
}

function eliminar_complementaria(data) {
    let respuesta = [];
    //se utiliza $.ajax(), a la cual se le pasa un objeto {}, con la información
    $.ajaxSetup({async: false});
    $.ajax({
        type: "POST", // la variable type guarda el tipo de la peticion GET,POST,..
        url: "/producto/eliminar_complementarias", //url guarda la ruta hacia donde se hace la peticion
...
function initTagify(tagList = null) {
    ...
    const elements = [
        ...
        '#kt_ecommerce_add_product_etiquetas'
    ];
    // Loop all elements
    elements.forEach(element => {
        // Get tagify element
...
//      img = elemento_imagen({"src": imagen.url_imagen});
            ...
            if (imagen.tipo_imagen == "complementaria") {
                let code = `<div class="col-2" id="contenedor-imagen-complementaria-${imagen.id_producto_imagen}">
                                                                        <div>
                                                                            <img src="${imagen.url_imagen}" width="100%">
                                                                        </div>
                                                                        <div>
                                                                            <a class="form-control primary" href="javascript:void(0);" onclick="eliminar_imagen_complementaria(this)" ruta_imagen="${imagen.url_imagen}" id="imagen-complementaria-${imagen.id_producto_imagen}">
                                                                                eliminar
                                                                            </a>
                                                                        </div>  
                                                                    </div>`;
...
function obtener_informacion() {
    ...
    let precio_oferta = $("#precio_oferta").val() ? $("#precio_oferta").val() : 0;
    ...
    let proveedores = $("#select_proveedores").val();
    let categorias = $("#select_categorias").val();
    let marcas = $("#select_marcas").val();
    let tags = obtener_lista_tags();
    let etiquetas = obtener_etiquetas();
    let atributos = obtener_atributos();
    let variantes = obtener_variantes();
    let sugeridos = obtener_sugeridos();
    let complementarios = obtener_complementarios();
    let id_variante = VARIANTE_REGISTRO;
    let productos_sugeridos = obtener_productos_sugeridos();
    let compra_venta = {
...
function ui_sugeridos(lista_atributos) {
    var tipo_atributo = "", valor_atributo = "", unidad_medida_atributo = "", descripcion_atributo = "";
    lista_atributos.map(function (atributo) {

        id_variante = atributo['id_producto'];
        id_producto_sugerido = atributo['id_producto_sugerido'];

        $("#body_kt_ecommerce_add_producto_sugerido").append(`<tr class="row_kt_ecommerce_add_producto_sugerido">
                                    <td class="p-3 id_variante">
...
function ui_complementos(lista_atributos) {
    var tipo_atributo = "", valor_atributo = "", unidad_medida_atributo = "", descripcion_atributo = "";
    lista_atributos.map(function (atributo) {

        id_variante = atributo['id_producto'];
        id_producto_sugerido = atributo['id_producto_complementario'];

        $("#body_kt_ecommerce_add_producto_complementario").append(`<tr class="row_kt_ecommerce_add_producto_complementario">
                                    <td class="p-3 id_variante">
                                        ${id_producto_sugerido}
                                    </td>
                                    <td class="p-3">
...
function agregar_sugerido_complemento(elemento) {
    console.log(elemento);
    var e = $(elemento)[0];
    var id = e.id;
    console.log(e);
    console.log(id);
    var id_tipo_producto = $('#id_' + id).val();
    console.log(id_tipo_producto);
//    var id_variante = $("#id_variante").val();
//    var principal = $("#principal").val();
    $("#body_" + id).append(`<tr class="row_${id}">
                                    <td class="p-3 id_variante">
                                        ${id_tipo_producto}
                                    </td>
                                    <td class="p-3">
...
function agregar_etiqueta(elemento) {
    console.log(elemento);
    var e = $(elemento)[0];
    var id = e.id;
    console.log(e);
    console.log(id);
    var id_tipo_producto = $('#id_' + id).val();
    var id_tipo_etiqueta = $('#select_' + id).val();
    console.log(id_tipo_producto);
//    var id_variante = $("#id_variante").val();
//    var principal = $("#principal").val();
    $("#body_" + id).append(`<tr class="row_${id}">
                                    <td class="p-3 etiqueta">
                                        ${id_tipo_producto}
                                    </td>
                                    <td class="p-3 tipo_etiqueta">
                                        ${id_tipo_etiqueta}
                                    </td>
                                    <td class="p-3">
                                        <p style="cursor:pointer;" onclick="eliminar_variante(this);">
...
function obtener_sugeridos() {
    var array_atributos = [], id_variante = "", principal = "", atributo = "";
    $(".row_kt_ecommerce_add_producto_sugerido").each(function () {
//        console.log($(this).find('.tipo_atributo'));
        sugerido = $(this).find('.id_variante') ? $($(this).find('.id_variante')).text().trim() : '';
//        console.log(tipo_atributo);
        atributo = {
            sugerido
        }
...
function obtener_complementarios() {
    var array_atributos = [], id_variante = "", principal = "", atributo = "";
    $(".row_kt_ecommerce_add_producto_complementario").each(function () {
//        console.log($(this).find('.tipo_atributo'));
        complemento = $(this).find('.id_variante') ? $($(this).find('.id_variante')).text().trim() : '';
//        console.log(tipo_atributo);
        atributo = {
            complemento
        }
...
function obtener_etiquetas() {
    var array_atributos = [], id_variante = "", principal = "", atributo = "";
    $(".row_kt_ecommerce_add_producto_etiqueta").each(function () {
//        console.log($(this).find('.tipo_atributo'));
        etiqueta = $(this).find('.etiqueta') ? $($(this).find('.etiqueta')).text().trim() : '';
        tipo_etiqueta = $(this).find('.tipo_etiqueta') ? $($(this).find('.tipo_etiqueta')).text().trim() : '';
//        console.log(tipo_atributo);
        atributo = {
            etiqueta,
            tipo_etiqueta
