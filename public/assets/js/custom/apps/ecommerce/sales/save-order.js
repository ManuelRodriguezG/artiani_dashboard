...
var KTAppEcommerceSalesSaveOrder = function () {
    ...
    const initSaveOrder = () => {
        ...
        $("#pedido_hora_inicial").flatpickr({
            enableTime: true,
            noCalendar: true,
            dateFormat: "H:i K",
        });
        $("#pedido_hora_final").flatpickr({
            enableTime: true,
            noCalendar: true,
            dateFormat: "H:i K",
        });
...
//                    KTDialer.createInstances();
        ...
        const calculateTotalProductos = (products) => {
            ...
            products.forEach(product => {
                ...
                // Add to total
                countPrice = parseFloat(countPrice + precio_total);
            });
...
//  ui_pedido_productos(productos.depurar);
    estatus_pedido = estatus_pedidos_listar();
//    console.log(estatus_pedido.depurar);
    ui_select_estatus_pedido(estatus_pedido.depurar);
    ...
    ui_select_tipos_entrega(tipos_entrega.depurar);
    KTAppEcommerceSalesSaveOrder.init();
});

function validacion_informacion_cliente(informacion) {
//    "cliente": {
...
//            "publico_general": publico_general,
    let secciones_errores = [], mensaje_error = "", error = false;
    if (!informacion.cliente.nombre) {
        secciones_errores.push("Nombres Cliente");
    }

    if (!informacion.cliente.apellido_paterno) {
        secciones_errores.push("Apellido Paterno Cliente");
    }

    if (!informacion.cliente.apellido_materno) {
        secciones_errores.push("Apellido Materno Cliente");
    }
    ...
    if (!informacion.cliente.contacto_1) {
        secciones_errores.push("Contacto1 Cliente");
    }

    if (secciones_errores.length > 0) {
        mensaje_error = "Campos " + secciones_errores.toString() + " de Información del cliente con información inválida.";
        error = true;
    }
    return {
        error,
...
function validacion_informacion_envio(informacion) {
//    "cliente": {
//            "nombre": cliente_nombres,
//            "apellido_paterno": cliente_apellido_paterno,
//            "apellido_materno": cliente_apellido_materno,
...
//                "horario_entrega": {
//                    "fecha_entrega": fecha,
//                    "hora_inicial_entrega": hora_inicial,
//                    "hora_final_entrega": hora_final,
//                    "turno_horario": turno
//                }
    let secciones_errores = [], mensaje_error = "", error = false;
    if (informacion.cliente.datos_envio.calle == null) {
        secciones_errores.push("Calle");
    }
    if (informacion.cliente.datos_envio.numero_exterior == null) {
        secciones_errores.push("Número exterior");
    }
    if (informacion.cliente.datos_envio.colonia == null) {
        secciones_errores.push("Colonia");
    }
    if (informacion.cliente.datos_envio.ciudad == null) {
        secciones_errores.push("Ciudad");
    }
    if (informacion.cliente.datos_envio.estado == null) {
        secciones_errores.push("Estado");
    }
    if (informacion.cliente.datos_envio.codigo_postal == null) {
        secciones_errores.push("Código postal");
    }
    if (informacion.cliente.datos_envio.horario_entrega.fecha_entrega == null) {
        secciones_errores.push("Fecha Entrega");
    }
    if (!informacion.cliente.datos_envio.horario_entrega.hora_inicial_entrega) {
        secciones_errores.push("Hora entrega");
    }

    if (secciones_errores.length > 0) {
        mensaje_error = "Campos " + secciones_errores.toString() + " de Información de Envío con información inválida.";
        error = true;
    }
    return {
        error,
...
function validacion_datos_facturacion(informacion) {
//    "cliente": {
//            "nombre": cliente_nombres,
//            "apellido_paterno": cliente_apellido_paterno,
//            "apellido_materno": cliente_apellido_materno,
...
//            }
    let secciones_errores = [], mensaje_error = "", error = false;
    if (informacion.cliente.datos_facturacion.facturar == 1) {
        if (informacion.cliente.datos_facturacion.uso_cfdi == null) {
            secciones_errores.push("Uso cfdi");
        }
        if (informacion.cliente.datos_facturacion.razon_social == null) {
            secciones_errores.push("Razón social");
        }
        if (informacion.cliente.datos_facturacion.rfc == null) {
            secciones_errores.push("Rfc");
        }
        if (informacion.cliente.datos_facturacion.regimen_fiscal == null) {
            secciones_errores.push("Regimen Fiscal");
        }
        if (informacion.cliente.datos_facturacion.estado == null) {
            secciones_errores.push("Estado Fiscal");
        }
        if (informacion.cliente.datos_facturacion.ciudad == null) {
            secciones_errores.push("Ciudad Fiscal");
        }
        if (informacion.cliente.datos_facturacion.colonia == null) {
            secciones_errores.push("Colonia Fiscal");
        }
        if (informacion.cliente.datos_facturacion.calle == null) {
            secciones_errores.push("Calle Fiscal");
        }
        if (informacion.cliente.datos_facturacion.numero_exterior == null) {
            secciones_errores.push("Número exterior Fiscal");
        }
        if (informacion.cliente.datos_facturacion.codigo_postal == null) {
            secciones_errores.push("Código Postal Fiscal");
        }
    }
    if (secciones_errores.length > 0) {
        mensaje_error = "Campos " + secciones_errores.toString() + " de Datos de Facturación con información inválida.";
        error = true;
    }
    return {
        error,
...
function validacion_detalles_pedido(informacion) {
//    "pedido": {
//            "productos": productos,
//            "descuento": descuento,
//            "estatus": $("#estatus_pedido").val(),
//            "envio": envio,
//            "subtotal": subtotal,
//            "total": total,
//            "tipo_entrega": {
...
//        }
    let secciones_errores = [], mensaje_error = "", error = false;
    if (informacion.pedido.estatus == -1 || informacion.pedido.estatus == 0) {
        secciones_errores.push("Estatus Pedido");
    }

    if (informacion.pedido.tipo_entrega.id_tipo_entrega == -1 || informacion.pedido.tipo_entrega.id_tipo_entrega == 0) {
        secciones_errores.push("Tipo Entrega");
    }

    if (secciones_errores.length > 0) {
        mensaje_error = "Campos " + secciones_errores.toString() + " del Pedido con información inválida.";
        error = true;
    }
    return {
        error,
...
//  }
//});
function ui_select_estatus_pedido(data) {
//    console.log(data);
    let opciones_tipos_entrega = [];
    opciones_tipos_entrega.push({
        value: -1,
        text: "Seleccionar estatus del pedido"
    });
...
//      alert("La clave es " + clave + " y el valor es " + json[clave]);
        if (data[clave].default == 1) {
            opciones_tipos_entrega.push({
                value: data[clave].id_estatus_pedido,
                text: data[clave].estatus_pedido,
                selected: "selected"
            });
        } else {
            opciones_tipos_entrega.push({
...
//    });
//  });
    console.log(opciones_tipos_entrega);
    rellenar_select("#estatus_pedido", opciones_tipos_entrega);
}
...
function funcion_crear_pedido(informacion) {
    let respuesta = crear_pedidoo(informacion);
}
...
function informacion_pedido() {
    ...
    let publico_general = $("#publico_general")[0].checked == true ? 1 : 0;
    ...
    let estatus_venta = $("#estatus_pedido option:selected").val() ? $("#estatus_pedido option:selected").val() : 0;
    ...
    let hora_inicial_entrega = obtener_hora_entrega($("#pedido_hora_inicial").val());
    let hora_final_entrega = obtener_hora_entrega($("#pedido_hora_final").val());
    let hora_inicial = hora_inicial_entrega.hora;
    let hora_final = hora_final_entrega.hora;
    let turno = hora_inicial.turno;
    //Totales
...
function obtener_cantidad_pago(elemento) {
    console.log($(elemento).parent().parent().children()[1]);
...
function obtener_productos_venta() {
    let productos = [], subtotal = 0, iva = 0, cantidad = 0, precio = 0, precio_total = 0;
    if ($(".producto-selected")) {
        $(".producto-selected").each(function (item) {
            console.log(item);
            console.log(this);
...
//              "producto": "Jaula para conejo amsterdam III",
            ...
            cantidad = parseInt($("#pedido-cantidad-" + this.value).val());
            precio = parseFloat(this.getAttribute("data-producto-precio"));
            precio_total = this.getAttribute('data-unidad-venta') > 1 ? (cantidad / this.getAttribute('data-factor')) * precio : precio * cantidad;
            subtotal = cantidad && precio_total ? precio_total : 0.00;
...
function ui_select_tipos_entrega(data) {
    console.log(data);
    let opciones_tipos_entrega = [];
    opciones_tipos_entrega.push({
        value: -1,
...
$("#tipos_entrega").change(function () {
    console.log($("#tipos_entrega option:selected").val());
    console.log($("#tipos_entrega option:selected").text());
    console.log(TIPOS_ENTREGA[$("#tipos_entrega option:selected").text()]);
//  if(TIPOS_ENTREGA[$("#tipos_entrega option:selected").val()].puntos_entrega)
    console.log(TIPOS_ENTREGA);
    let opciones_puntos = [];
...
function modal_cambiar_precio(elemento) {
//span_precio_ (12)
    let id_producto = elemento.id.substring(12);
    console.log(id_producto);
    console.log(elemento.innerText);
    let precio = parseFloat(elemento.innerText);
    $("#cambiar_precio_nuevo").val(precio);
    $(".boton-modificar-precio")[0].id = "button_modificar_precio_" + id_producto;
    $("#kt_modal_cambiar_precio").modal("show");
    console.log("cambiar_precio");
}

function modificar_precio_producto(elemento) {
//button_modificar_precio_ (24)
    let id_producto = elemento.id.substring(24);
    let target_precio = $("#kt_ecommerce_edit_order_selected_products #span_precio_" + id_producto);
    let precio_nuevo = parseFloat($("#cambiar_precio_nuevo").val());
    let precio_antiguo = target_precio.text();
    let target_input_precio = $("#producto-seleccionado-" + id_producto);
    //producto-seleccionado- (22)
    target_input_precio.attr("data-producto-precio", precio_nuevo);
    target_precio.text(precio_nuevo)
    console.log(id_producto);
    console.log(precio_nuevo);
    console.log(precio_antiguo);
    target_precio.text(precio_nuevo);
    detectarProductos();
}
...
function crear_pedidoo(data) {
    console.log(JSON.stringify(data));
    $.ajaxSetup({async: false});
//se utiliza $.ajax(), a la cual se le pasa un objeto {}, con la información
    $.ajax({
        type: "POST", // la variable type guarda el tipo de la peticion GET,POST,..
        url: "/ventas/registrar", //url guarda la ruta hacia donde se hace la peticion
...
// Calculate total cost
const calcularTotalProductos = (products) => {
    let countPrice = 0;
    const descuento = parseFloat(document.querySelector('[id="kt_ecommerce_edit_order_descuento_price"]').innerText);
    const envio = parseFloat(document.querySelector('[id="kt_ecommerce_edit_order_envio_price"]').innerText);
    // Loop through all selected prodcucts
