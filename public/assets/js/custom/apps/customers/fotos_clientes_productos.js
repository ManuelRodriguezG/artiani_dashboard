                    ...
                    if (status == 'Valid') {
                        ...
                        if ($("#avatar")[0].files.length > 0) {
                            ...
                            if (respuesta.error == false) {
                                ...
                                let response = guardar_productos_imagen(informacion);
...
function guardar_productos_imagen(data) {
    let respuesta = [];
...
//    contentType: 'multipart/form-data',
        url: "/clientes/guardar_productos_imagen", //url guarda la ruta hacia donde se hace la peticion
...
async function cargar_imagen_compra(data) {
    ...
    //se utiliza $.ajax(), a la cual se le pasa un objeto {}, con la información
//  $.ajaxSetup({async: false});
    respuesta = await fetch("/clientes/actualizar_portada", {method: 'POST', body: data})
...
function obtener_informacion(url) {
    ...
    let titulo = $("#id_titulo").val() ? $("#id_titulo").val() : '';
