                ...
                validator.validate().then(async function (status) {
                    ...
                    if (status == 'Valid') {
                        ...
                        if ($("#avatar")[0].files.length > 0) {
                            ...
                            formSubirPortada.append('img', imgPortada);
                            let respuesta = await cargar_imagen_compra(formSubirPortada);
                            console.log(respuesta.error);
                            if (respuesta.error == false) {
                                //second step
                                let url_imagen_cliente_compra = respuesta.depurar;
                                let informacion = obtener_informacion(url_imagen_cliente_compra);
                                let response = alta_sucursal(informacion);
...
//    contentType: 'multipart/form-data',
        url: "/sucursal/alta_sucursal", //url guarda la ruta hacia donde se hace la peticion
...
async function cargar_imagen_compra(data) {
    let respuesta = [];
    //se utiliza $.ajax(), a la cual se le pasa un objeto {}, con la información
//  $.ajaxSetup({async: false});
    respuesta = await fetch("/sucursal/registrar_imagen", {method: 'POST', body: data})
...
function registrar_productos_imagen(data) {
    let respuesta = [];
    //se utiliza $.ajax(), a la cual se le pasa un objeto {}, con la información
    $.ajaxSetup({async: false});
    $.ajax({
        type: "POST", // la variable type guarda el tipo de la peticion GET,POST,..
//    contentType: 'multipart/form-data',
        url: "/clientes/registrar_productos_imagen_portada", //url guarda la ruta hacia donde se hace la peticion
        data: data, // data recive un objeto con la informacion que se enviara al servidor
        success: function (datos) { //success es una funcion que se utiliza si el servidor retorna informacion
            respuesta = JSON.parse(datos);
        }
...
function obtener_informacion(url) {

    
    let nombre = $("#id_nombre").val() ? $("#id_nombre").val() : '';
    let pais = $("#id_pais").val() ? $("#id_pais").val() : '';
    let ciudad = $("#id_ciudad").val() ? $("#id_ciudad").val() : '';
    let colonia = $("#id_colonia").val() ? $("#id_colonia").val() : '';
    let codigo_postal = $("#id_codigo_postal").val() ? $("#id_codigo_postal").val() : '';
    let calle = $("#id_calle").val() ? $("#id_calle").val() : '';
    let numero_exterior = $("#id_numero_exterior").val() ? $("#id_numero_exterior").val() : '';
    let numero_interior = $("#id_numero_interior").val() ? $("#id_numero_interior").val() : '';
    let incrustado = $("#id_incrustado").val() ? $("#id_incrustado").val().replace(/"/g, "\'") : '';
...
//        console.log(tipo_atributo);
        ...
        array_atributos.push(id_variante);
    });
