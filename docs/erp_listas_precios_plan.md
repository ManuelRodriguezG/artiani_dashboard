...
- Costo comercial fallback 2026-08-18: la mesa de productos ya no depende solo de `erp_catalogo_skus.costo_referencia`. Para margen usa prioridad read-only: costo promedio ponderado de inventario, costo ultimo de proveedor activo/preferido, ultima compra no cancelada y catalogo solo como fallback temporal. La UI muestra la fuente del costo para evitar margenes opacos; no actualiza costos contables ni escribe BD.
