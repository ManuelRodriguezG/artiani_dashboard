...
- Costo comercial central 2026-08-19: la mesa de productos y las revisiones/dry-run de Listas consumen `RentabilidadErp::resolverCostoVigenteSku` en modo read-only. El margen puede usar costos directos, presentaciones derivadas, aperturas confirmadas y paquetes/combos con formula y confianza; `erp_catalogo_skus.costo_referencia` queda solo como fallback historico auditado. UAT: `storage/uat/uat_listas_precios_costos_rentabilidad_readonly.php`.
...
- Validacion producto operativo 2026-08-15: la mesa de precios solo muestra y permite guardar precios para SKUs activos cuyo producto maestro tambien este `activo`. Si un producto esta `inactivo`, no entra a listas de precios hasta que Catalogo lo reactive; esto mantiene la captura enfocada en productos operativos y evita agrandar listas con articulos fuera de operacion.
- Paginacion de productos 2026-08-14: la mesa pagina el catalogo completo con `pagina`, `por_pagina`, `total_disponible` y `total_paginas`. El operador puede navegar todos los productos sin un limite oculto, usando busqueda/filtros cuando el catalogo sea grande.
- Guardado masivo visible 2026-08-14: `Guardar cambios` queda disponible tambien en la barra principal de Captura. Guarda unicamente los SKUs modificados en estado, no todos los productos de la pagina; los cambios se conservan al navegar entre paginas de productos ya visitadas para capturar lote operativo.
...
- Publico general sin cliente 2026-08-22: una lista para todos no debe asignarse a un cliente ficticio ni pedir IDs. Se configura desde `Alcance` con canal `general`, sin almacen si aplica globalmente, precios activos y estatus `activa`. Cliente directo, lista default CRM y segmento CRM son reglas mas especificas que ganan sobre la lista general cuando existan.
